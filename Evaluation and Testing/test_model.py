...
checkpoint = torch.load('./save/halo_model2', map_location=torch.device(device), weights_only=False)
...
# optimizer.load_state_dict(checkpoint['optimizer'])  # Commented out - not needed for testing
...
checkpoint_path = './results/datasets/haloDataset_checkpoint.pkl'
start_idx = 0
if os.path.exists(checkpoint_path):
  synthetic_ehr_dataset = pickle.load(open(checkpoint_path, 'rb'))
  start_idx = len(synthetic_ehr_dataset)
  print(f'Resuming from checkpoint with {start_idx} patients')

total_batches = len(range(0, len(train_ehr_dataset), config.sample_batch_size))
for batch_num, i in enumerate(tqdm(range(start_idx, len(train_ehr_dataset), config.sample_batch_size))):
  ...
  if (batch_num + 1) % 50 == 0:
    pickle.dump(synthetic_ehr_dataset, open(checkpoint_path, 'wb'))
    print(f'  Checkpoint saved: {len(synthetic_ehr_dataset)} patients')

pickle.dump(synthetic_ehr_dataset, open(f'./results/datasets/haloDataset.pkl', 'wb'))
print(f'Final dataset saved: {len(synthetic_ehr_dataset)} patients')
